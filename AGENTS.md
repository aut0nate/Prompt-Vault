...
- `docker-compose.prod.yaml` pulls `ghcr.io/aut0nate/prompt-vault:${IMAGE_TAG}` and mounts `./storage` to `/app/data`; `IMAGE_TAG` is required and should be a published commit SHA.
